...
export function* loadCartFlow(action) {
	try {
		...
		if (includeProducts) {
			...
			yield all(productIds.map(productId => put(loadProduct(productId))));
		}
...
export function* confirmPaymentFlow(action) {
	...
	try {
		...
		const {name, email, country, state, zip, cc, expYear, expMonth, cvv} = action.payload;

		const order = yield call(Api.confirmPayment, name, email, country, state, zip, cc, expYear, expMonth, cvv);
